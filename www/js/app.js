...
var todoApp = angular.module('Todo-ionic', ['ionic', 'ngCordova']);
...
todoApp.controller('ConfigController', function ($scope, $ionicPlatform, $ionicLoading, $location, $ionicHistory, $cordovaSQLite) {
    ...
    $ionicPlatform.ready(function () {
        ...
        if (window.cordova) {
            $window.plugins.sqlDB.copy('TodoList.sqlite', function () {
                db = $cordovaSQLite.openDB("TodoList.sqlite");
            ...
            db.transaction(function (tx) {
                ...
                tx.executeSql("INSERT INTO tblCategories (category_name) VALUES (?)", ["Groceries"]);
                tx.executeSql("INSERT INTO tblCategories (category_name) VALUES (?)", ["Bills"]);
                tx.executeSql("INSERT INTO tblCategories (category_name) VALUES (?)", ["Dogs"]);
                tx.executeSql("INSERT INTO tblCategories (category_name) VALUES (?)", ["Weekend"]);
            });
            $location.path('/categories');
            $ionicLoading.hide();
        }
...
todoApp.controller('ListsController', function ($scope, $ionicPlatform, $ionicPopup, $cordovaSQLite, $stateParams) {
    ...
    $scope.delete = function (item) {
        var itemsQuery = "DELETE FROM tblTodoListItems where todo_list_id = ?";
        var listQuery = "DELETE FROM tblTodoLists where id = ?";
        $cordovaSQLite.execute(db, itemsQuery, [item.id]).then(function (res) {
            $cordovaSQLite.execute(db, listQuery, [item.id]).then(function (res) {
